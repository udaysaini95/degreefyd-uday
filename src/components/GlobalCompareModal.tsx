        ...
        <>
            ...
            >
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 shrink-0">
                    ...
                    <button
                        onClick={onClose}
                        className="p-1 border border-gray-200 rounded hover:bg-gray-50 flex items-center justify-center"
                    >
                ...
                {/* Cards Row */}
                <div className="flex flex-row items-stretch gap-0 px-4 pt-5 pb-4 relative">
                    {/* College 1 Card */}
                    <div className="flex-1">
                        {college1 ? (
                            <MobileFilledCard college={college1} onEdit={() => setModalFor(1)} />
                        ) : (
                            <MobileEmptyCard onAdd={() => setModalFor(1)} />
                        )}
                    </div>

                    {/* VS Badge */}
                    <div className="flex items-center justify-center px-2 shrink-0 z-10">
                        <div className="w-8 h-8 rounded-full bg-[#FF8C00] border-2 border-white shadow flex items-center justify-center">
                    ...
                    <div className="flex-1">
                        {college2 ? (
                            <MobileFilledCard college={college2} onEdit={() => setModalFor(2)} />
                        ) : (
                            <MobileEmptyCard onAdd={() => setModalFor(2)} />
                        )}
                    </div>
                </div>
                ...
                <div className="px-4 pb-6 pt-1 shrink-0">
                    <button
                        ...
                        className={`w-full py-3 rounded-lg text-[15px] font-semibold transition-all ${bothSelected
                                ? "bg-[#113654] text-white hover:bg-[#0D2A43]"
                                : "bg-[#c8cfd5] text-white cursor-not-allowed"
                            }`}
                    >
                        Compare
                    </button>
            ...
            <div
                ...
                `}
            >
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-[650px] overflow-hidden relative">
                    {/* Header */}
...
function MobileEmptyCard({ onAdd }: { onAdd: () => void }) {
    return (
        <div
            onClick={onAdd}
            className="flex flex-col items-center justify-center border border-dashed border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors bg-white gap-2 py-6 px-3 h-full min-h-[150px]"
        >
            ...
            <span className="text-[12px] font-semibold text-[#1a2e4a] text-center">Add College</span>
        </div>
    );
}

function MobileFilledCard({ college, onEdit }: { college: College; onEdit: () => void }) {
    return (
        <div className="flex flex-col items-center p-3 border border-gray-200 rounded-lg bg-white shadow-sm h-full min-h-[150px]">
            {/* Logo */}
            <div className="w-[70px] h-[36px] border border-gray-100 rounded-md flex items-center justify-center p-1 mb-2 bg-white shrink-0">
                <img
                    src={college.logo}
                    alt={college.name}
            ...
            {/* Name + Location */}
            <div className="flex flex-col items-center gap-0.5 w-full mb-auto">
                <h3 className="text-center font-bold text-[#1a2e4a] text-[12px] leading-tight line-clamp-2">
                    {college.name}
                </h3>
                <div className="flex items-center gap-0.5 text-gray-500 text-[10px] mt-0.5">
            ...
            <button
                onClick={onEdit}
                className="w-full mt-3 py-1.5 border border-[#8e9caf] text-[#1a2e4a] rounded-[4px] text-[11px] font-medium hover:bg-gray-50 transition-colors flex items-center justify-center gap-1"
            >
                Edit
...
/* ─── Desktop Card Components (unchanged) ─── */
